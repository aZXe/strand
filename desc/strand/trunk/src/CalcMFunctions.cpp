...
// Число
          if (isdigit(*p) || (*p == '-' && isdigit(*(p + 1)))) {
            s = t;
//            s->A = new double(AnsiString(p).ToDouble());       ///
...
//---------------------------------------------------------------------------
void TMCalculate::PrintTreeTmp(Node *A, FILE *f)
{
  fprintf(f, "Node : %ld\n", (long)A);
  ...
  if (A->pA) PrintTreeTmp(A->pA, f);
  if (A->pB) PrintTreeTmp(A->pB, f);
}
//---------------------------------------------------------------------------
// Подсчет результата
