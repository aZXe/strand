...
//double Report[size_rep];
TDoubleArray Report;//[size_rep];
                     ...
                     TDoubleArray &f, double &eps, bool isQuantiles)
{
  ...
  if (ConfigForm->rbLogNormal->Checked) {
    ...
    VectorDualSort(wX, wW, SORT_ASCENDING);
  }
  else
    VectorDualSort(X, W, SORT_ASCENDING);
    ...
    for (i = 0; i < c - 1; i++) {
      if (X1[i] == X1[i + 1]) {
        for (ii = i + 1; ii < c; ii++)
          if (X1[ii] != X1[i]) break;
        if (ii == c) t = 0;
        else t = (X1[ii] - X1[i]) / (exp(X1[ii]) - exp(X1[i]));
        for (jj = i; jj < ii; jj++)
          f[jj] *= t;
        i = ii;
      }
      else
        f[i] *= (X1[i + 1] - X1[i]) / (exp(X1[i + 1]) - exp(X1[i]));
    }
    f[c - 1] = 0;

    long LnN = (long)floor(log(N));
    for (i = 0; i < LnN; i++) f[i] = 0;
    for (i = LnN; i < c - LnN - 1; i++) {
      f[i] = VectorSum(f, i - LnN, 2 * LnN + 1);
      f[i] /= 2 * LnN + 1;
    }
    for (i = c - LnN - 1; i < c; i++) f[i] = 0;
