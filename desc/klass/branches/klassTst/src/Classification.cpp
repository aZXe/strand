...
double alpha = 1.3, betta = 1, gamma = 0.2, delta = 1.1, phi = 0.5,
       theta = 1.75, psi = 1.5, c_alpha = 2, p_max1 = 0.90, p_max2 = 0.95;
...
//---------------------------------------------------------------------------
double metrica(TDoubleArray &X, TDoubleArray &Y, TDoubleArray &W, bool isMayBeNull)
{
  int i, N = min(X.Length, Y.Length);
  double max = -1, s = 0;
  ...
  for (i = 0; i < N; i++) {
    s += W[i] * fabs(X[i] - Y[i]);
//    if (fabs(X[i] - Y[i]) > max && W[i] != 0) max = fabs(X[i] - Y[i]);
  }
////  s = sqrt(s);
...
void MakeM1M2(TDoubleArray &C, TDoubleArray &R1, TIntSet &M1, TIntSet &M2)
{
  ...
  VectorSet(RR1, 0);
  for (i = 4; i < N - 4; i++)
    RR1[i] = ((tR1[i - 4] + tR1[i - 3] + tR1[i - 2] + tR1[i - 1]) /
              (tR1[i + 3] + tR1[i + 2] + tR1[i + 1] + tR1[i] + 1));

  tRR1 = RR1.Copy();
  VectorSort(tRR1, SORT_ASCENDING);
  max1 = tRR1[tRR1.Length * p_max1];
  max2 = tRR1[tRR1.Length * p_max2];
...
//---------------------------------------------------------------------------
void Classification(T2DoubleArray &A, TDoubleArray &W, TintIntSetMap &Classes, TIntArray &IK, int k, int count_vars)
{
  ...
  Weights = W;
  alpha   = StrToFloat(ParamsForm->Edit1->Text);
  betta   = StrToFloat(ParamsForm->Edit2->Text);
  gamma   = StrToFloat(ParamsForm->Edit3->Text);
  delta   = StrToFloat(ParamsForm->Edit4->Text);
  phi     = StrToFloat(ParamsForm->Edit5->Text);
  theta   = StrToFloat(ParamsForm->Edit6->Text);
  psi     = StrToFloat(ParamsForm->Edit7->Text);
  c_alpha = StrToFloat(ParamsForm->Edit8->Text);
  p_max1  = StrToFloat(ParamsForm->Edit9->Text);
  p_max2  = StrToFloat(ParamsForm->Edit10->Text);
...
// 1.
  ...
  if (ParamsForm->cbFirstEtap->Checked) {
    MakeM1M2(C, R1, M1, M2);
